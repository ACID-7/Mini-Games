...
function ensureRoundState(room) {
  const gs = room.gameState || (room.gameState = {});
  ...
  switch (room.mode) {
    ...
    case 'chain_reaction':
      gs.history ||= [];
      gs.votes ||= {};
      gs.turnOrder ||= playerOrder(room);
      if (typeof gs.turnIndex !== 'number') gs.turnIndex = 0;
      gs.baseSentence ||= randomChoice(BASE_SENTENCES);
      gs.currentSentence ||= gs.baseSentence;
      break;
    case 'secret_objective':
      gs.assignment ||= {};
      gs.chat ||= [];
    ...
    case 'one_word_story_war':
      ...
      gs.votes ||= {};
      if (typeof gs.turnIndex !== 'number') gs.turnIndex = 0;
      gs.start ||= randomChoice(STORY_STARTS);
      break;
    case 'suspicious_one':
      gs.submissions ||= {};
      gs.entries ||= [];
      ...
      break;
    case 'memory_trap':
      gs.prompts ||= sample(MEMORY_PROMPTS, 2);
      if (typeof gs.currentPromptIndex !== 'number') gs.currentPromptIndex = 0;
      gs.answersByPlayer ||= {};
      gs.trapCards ||= [];
      gs.votes ||= {};
      gs.alteredCardId ||= '';
...
function countPlayersWithObjectValue(room, source = {}) {
  return room.players.filter((player) => source[player.id] !== undefined).length;
}

function countPlayersMatching(room, predicate) {
  return room.players.filter((player) => predicate(player.id)).length;
}

function shouldAutoAdvance(room) {
  const gs = room.gameState || {};
  const playerCount = room.players.length;

  if (room.status !== 'playing' || room.phase === 'reveal' || room.phase === 'game_over') {
    return false;
  }
  ...
  switch (room.mode) {
    case 'last_message':
    case 'suspicious_one':
      if (room.phase === 'writing') return countPlayersWithObjectValue(room, gs.submissions) >= playerCount;
      if (room.phase === 'voting') return countPlayersWithObjectValue(room, gs.votes) >= playerCount;
      return false;
    case 'rule_breaker':
      if (room.phase === 'prompting') {
        const promptKey = `p${gs.currentPromptIndex}`;
        return countPlayersWithObjectValue(room, gs.answersByPrompt?.[promptKey]) >= playerCount;
      }
      if (room.phase === 'outsider_guess') return Boolean(gs.outsiderGuess);
      if (room.phase === 'voting') return countPlayersWithObjectValue(room, gs.votes) >= playerCount;
      return false;
    case 'fake_internet':
      if (room.phase === 'writing') return countPlayersWithObjectValue(room, gs.fakePosts) >= playerCount;
      if (room.phase === 'voting') return countPlayersWithObjectValue(room, gs.votes) >= playerCount;
      return false;
    case 'chain_reaction':
      if (room.phase === 'voting') return countPlayersWithObjectValue(room, gs.votes) >= playerCount;
      return false;
    case 'secret_objective':
      if (room.phase === 'guessing') return countPlayersWithObjectValue(room, gs.guesses) >= playerCount;
      return false;
    case 'speed_lies':
      if (room.phase === 'answering') return countPlayersWithObjectValue(room, gs.answers) >= playerCount;
      if (room.phase === 'voting') return countPlayersWithObjectValue(room, gs.votes) >= playerCount;
      return false;
    case 'voice_swap':
      if (room.phase === 'recording') return countPlayersWithObjectValue(room, gs.recordings) >= playerCount;
      if (room.phase === 'guessing') return countPlayersWithObjectValue(room, gs.guesses) >= playerCount;
      return false;
    case 'one_word_story_war':
      if (room.phase === 'voting') return countPlayersWithObjectValue(room, gs.votes) >= playerCount;
    ...
    case 'memory_trap':
      if (room.phase === 'collecting') {
        return countPlayersMatching(
          room,
          (playerId) => gs.answersByPlayer?.[playerId]?.[gs.currentPromptIndex] !== undefined,
        ) >= playerCount;
      }
      if (room.phase === 'voting') return countPlayersWithObjectValue(room, gs.votes) >= playerCount;
      return false;
...
function autoAdvanceIfReady(room) {
  let guard = 0;
  while (shouldAutoAdvance(room) && guard < 6) {
    advancePhase(room);
    guard += 1;
  }
}
...
function initRound(room) {
  ...
  switch (room.mode) {
    case 'last_message': {
      ...
      const eligibleOddPlayers =
        ids.length > 1
          ? ids.filter((id) => id !== previousGameState.oddPlayerId)
          : ids;
      room.phase = 'writing';
      room.banner = `Round ${room.currentRound}/${room.rounds}: write your last message.`;
      room.gameState = {
        pair,
        oddPlayerId: randomChoice(eligibleOddPlayers.length ? eligibleOddPlayers : ids),
        submissions: {},
        entries: [],
        votes: {},
        scored: false
...
function handlePlayerAction(room, playerId, payload) {
  ...
  autoAdvanceIfReady(room);
}

function buildView(room, viewerId) {
  const gs = room.gameState || {};
  const me = getPlayer(room, viewerId);
  const votesCount = Object.keys(gs.votes || {}).length;
  const guessesCount = Object.keys(gs.guesses || {}).length;
  const base = {
    code: room.code,
    hostId: room.hostId,
    status: room.status,
