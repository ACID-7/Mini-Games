...
function handlePlayerAction(room, playerId, payload) {
  ...
  switch (room.mode) {
    ...
    case 'rule_breaker':
      ...
      if (room.phase === 'voting' && type === 'votePlayer') gs.votes[playerId] = payload.playerId;
      break;
    case 'fake_internet':
...
async function createRoom(store, name) {
  ...
  const room = {
    ...
    players: [{ id: playerId, name: safeText(name, 20) || 'Player', score: 0 }],
    gameState: {}
  };
...
async function joinRoom(store, name, code) {
  ...
  const playerId = makeId('player');
  room.players.push({ id: playerId, name: safeText(name, 20) || 'Player', score: 0 });
  room.banner = `${safeText(name, 20) || 'A player'} joined the room.`;
  await store.saveRoom(room);
...
async function leaveRoom(store, roomCode, playerId) {
  ...
  if (room.players.length < 3 && room.status === 'playing') {
    ...
    room.phase = 'lobby';
    room.gameState = {};
    room.banner = 'Returned to lobby because fewer than 3 players remain.';
    resetScores(room);
  }
